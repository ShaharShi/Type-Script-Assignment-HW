...
function checkExpirationDate(productDate) {
    ...
    switch (true) {
        ...
        case Number(productYear) > year:
            {
                result = 'The Product Has Not Expired';
            }
            break;
        case Number(productMon) < mon:
            {
                result = 'Product Expire';
            }
            break;
...
function checkWhosMoreExpensive() {
    ...
    if (isNaN(objOne.price) || isNaN(objTwo.price))
        return;
    switch (true) {
        case objOne.price == objTwo.price:
            {
                result = `${objOne.name} And ${objTwo.name} Are Equal in Price !`;
            }
            break;
        case objOne.price > objTwo.price:
            {
                result = `${objOne.name} is more Expensive then ${objTwo.name}`;
            }
            break;
        case objOne.price < objTwo.price:
            {
                result = `${objTwo.name} is more Expensive then ${objOne.name}`;
            }
            break;
        default: break;
    }
